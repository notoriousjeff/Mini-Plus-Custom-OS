	...
	while (SDL_PollEvent(&event))
	{
		...
		if (event.type == SDL_KEYDOWN || event.type == SDL_KEYUP)
		{
			...
			if (key == MINUI_A)
				...
				i = kButtonY;
			else if (key == MINUI_START)
				i = kButtonStart;
			else if (key == MINUI_SELECT)
				i = kButtonSelect;
			else if (key == MINUI_UP)
				i = kButtonUp;
			else if (key == MINUI_DOWN)
				i = kButtonDown;
			else if (key == MINUI_LEFT)
				i = kButtonLeft;
			else if (key == MINUI_RIGHT)
				i = kButtonRight;
			else if (key == MINUI_L)
				i = kButtonL;
			else if (key == MINUI_R)
				i = kButtonR;
			else if (key == MINUI_MENU)
				i = kButtonMenu;
			else if (key == MINUI_L2)
				i = kButtonL2;
			else if (key == MINUI_R2)
				i = kButtonR2;
			else if (key == MINUI_L3)
				i = kButtonL3;
			else if (key == MINUI_R3)
				i = kButtonR3;
			else if (key == MINUI_POWER)
				i = kButtonPower;
			else if (key == MINUI_VOLDN)
				i = kButtonVolDn;
			else if (key == MINUI_VOLUP)
				i = kButtonVolUp;
			else
				continue;
...
static SDL_Color blue = {BLUE_TRIAD};
...
static void trimSortingMeta(char **str)
{ // eg. `001) `
	char *safe = *str;
	while (isdigit(**str))
...
void GFX_blitMenu(SDL_Surface *surface, char *name, char *path, char *unique, int row, int selected_row)
{
	...
	if (row == selected_row)
	{
		...
		// bar
		SDL_FillRect(surface, &(SDL_Rect){0, Screen.main.list.y + (row * Screen.main.list.row_height), Screen.width, Screen.main.list.row_height}, SDL_MapRGB(surface->format, BLUE_TRIAD));
...
#define kTextBoxMaxRows 16
	...
	{
		if (row_count + 1 >= kTextBoxMaxRows)
			return; // TODO: bail
	...
	for (int i = 0; i < row_count; i++)
	{
		...
		if (len)
		{
			text = TTF_RenderUTF8_Blended(font_l, line, blue);
			int x = ox;
			x += (width - text->w) / 2;
...
int GFX_blitText(SDL_Surface *surface, char *str, int size, int x, int y, int width, int color, int has_shadow)
{
	...
	text = TTF_RenderUTF8_Blended(font, str, has_shadow ? shadow_color : (color == -1 ? disabled : (color ? blue : white)));
	int w = text->w;
	...
	if (has_shadow)
	{
		...
		SDL_FreeSurface(text);
		text = TTF_RenderUTF8_Blended(font, str, color == -1 ? gray : (color ? blue : white));
	}
...
{
	TTF_Font *font = size == 0 ? font_s : (size == 1 ? font_m : font_l);
	return TTF_RenderUTF8_Blended(font, text, color ? blue : white);
}

void GFX_blitBattery(SDL_Surface *surface, int x, int y)
{
	if (isCharging())
		SDL_BlitSurface(battery_charging, NULL, surface, &(SDL_Rect){x, y});
	else
	{
...
{
	if (x == Screen.menu.settings.x)
		GFX_blitWindow(surface, x, y, Screen.settings.width, Screen.settings.height, 0);
