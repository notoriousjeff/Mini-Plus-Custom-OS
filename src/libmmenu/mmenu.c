...
static MenuReturnStatus SaveLoad(char *rom_path, char *save_path_template, SDL_Surface *optional_snapshot, MenuRequestState requested_state, AutoSave_t autosave)
{
	...
	if (requested_state == kRequestSave)
	{
		if (!optional_snapshot)
			optional_snapshot = SDL_GetVideoSurface();
	...
	else if (requested_state == kRequestLoad)
	{
		...
		if (exists(save_path) && total_discs)
		{
			...
			if (slot_disc_name[0] == '/')
				...
				sprintf(slot_disc_path, "%s%s", base_path, slot_disc_name);
			char *disc_path = disc_paths[disc];
...
{
	if (requested_state == kRequestSave || requested_state == kRequestLoad)
		return SaveLoad(rom_path, save_path_template, optional_snapshot, requested_state, autosave);
	...
	int blue_rgb = SDL_MapRGB(screen->format, BLUE_TRIAD);
	int gray_rgb = SDL_MapRGB(screen->format, DISABLED_TRIAD);
	SDL_Color blue = (SDL_Color){BLUE_TRIAD};
	...
	if (requested_state != kRequestMenu)
	{ // sleep or poweroff
		if (disable_poweroff && requested_state == kRequestPowerOff)
			return kStatusContinue;
		SystemRequest(requested_state);
		if (requested_state == kRequestPowerOff)
			return kStatusPowerOff;
	}
...
#define kSleepDelay 30000
		if (now - cancel_start >= kSleepDelay && preventAutosleep())
			cancel_start = now;
		...
		if (dirty)
		{
			...
			for (int i = 0; i < kItemCount; i++)
			{
				...
				if (i == selected)
				{
					int bg_color_rgb = disabled ? gray_rgb : blue_rgb;
			...
			else if (state_support && (selected == kItemSave || selected == kItemLoad))
			{
				...
				else
				{
					...
					if (save_exists)
					{ // has save but no preview
						SDL_BlitSurface(no_preview, NULL, screen, &(SDL_Rect){Screen.menu.preview.x + Screen.menu.preview.inset + (hw - no_preview->w) / 2, Screen.menu.preview.y + Screen.menu.preview.inset + (hh - no_preview->h) / 2});
					}
					else
					{ // no save
						SDL_BlitSurface(empty_slot, NULL, screen, &(SDL_Rect){Screen.menu.preview.x + Screen.menu.preview.inset + (hw - empty_slot->w) / 2, Screen.menu.preview.y + Screen.menu.preview.inset + (hh - empty_slot->h) / 2});
					}
				}
